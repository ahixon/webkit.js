...
# Files get into here through derived.gypi, the files included
{
  ...
  'targets': [
	...
	},
	{
		...
		'sources': ['<@(curl)',],
		'depends': ['mbedtls'],
		'sources/': [['exclude','(docs/|m4/|packages/|perl/|tests/|vs/|winbuild/|CMake/|tool_|macos/)'],],
		'include_dirs': [
			'<(DEPTH)/deps/zlib',
			'<(DEPTH)/deps/curl',
			'<(DEPTH)/deps/curl/include',
			'<(DEPTH)/deps/curl/lib',
			'<(DEPTH)/deps/mbedtls/include',
			'<(DEPTH)/deps/mbedtls/lib',
		],
		'cxx':'<(emscripten_cc)',
	...
	{
		'target_name':'mbedtls',
		'sources':['<@(mbedtls_files)'],
		'include_dirs':['<(DEPTH)/deps/mbedtls/include'],
		'cxx':'<(emscripten_cc)',
		'cflags+':[''],
	},
